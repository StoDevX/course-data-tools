...
def find_days(daystring):
	...
	if '-' in daystring:
		...
		listOfDays = DAY_SEQUENCE[DAY_SEQUENCE.index(startDay):DAY_SEQUENCE.index(endDay) + 1]
	else:
...
def find_time(timestring):
	...
	start, end = [clean_timestring_segment(segment) for segment in cleanedTimestring.split('-')]
