...
def parse_prerequisites(course):
	if course['desc'] and 'Prerequisite' in course['desc']:
		desc = course['desc']
...
def clean_course(course):
	...
	if course['gereqs']:    course['gereqs']    = parse_links_for_text(course['gereqs'])
	if course['locations']: course['locations'] = parse_links_for_text(course['locations'])
	if course['times']:     course['times']     = parse_paragraph_as_list(course['times'])
