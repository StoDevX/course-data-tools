...
def create_offerings(*, times, locations):
    ...
    if times and locations and len(times) != len(locations) and len(locations) != 1:
        raise UserWarning(
            f"Times ({len(times)}) and Locations ({len(locations)}) are different lengths in",
            {"times": times, "locations": locations},
            {"originals": [o_times, o_locations]},
        )
...
def clean_course(course):
    ...
    if course["coursesubtype"] and course["coursesubtype"] in course_types:
        ...
        raise UserWarning(
            f"'{course['type']}' doesn't appear in the types list, in", course
        )
